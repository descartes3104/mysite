...
def file_speech_to_text(audio_file, set_language):
    ...
    with sr.AudioFile(audio_file) as source:
        audio = recognizer.record(source)  # sr.Recognizer().record(開いた音声ファイル)で認識準備

    try:
        text = recognizer.recognize_google(audio, language=set_language_list[set_language])  # sr.Recognizer().recognize_google(音声データ,言語)で音声認識して、textに代入
    except sr.UnknownValueError:
        text = "音声認識に失敗しました"
    except sr.RequestError as e:
        text = f"音声認識サービスに接続できませんでした; {e}"
...
def mic_speech_to_text(set_language):
    ...
    with sr.Microphone() as source:
        audio = recognizer.listen(source)  # sr.Recognizer().listen(マイク入力)で認識準備
